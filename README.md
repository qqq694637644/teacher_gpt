...
Startup fails when the file is missing, uses another data version, contains unknown fields, has incomplete page coverage, has broken references, or does not represent a complete index.
...
The verified source currently yields 434 layout heading candidates: 102 numbered headings and 332 unnumbered candidates. Two unnumbered author-name headings occur before any printed section and are excluded from section coverage.

### 2. Review the complete Manifest
...
The Manifest must cover the complete book. It stores printed sections and ordered learning-unit trees. Project IDs are generated by the compiler, not authored in YAML.

Every page retrieval step must be marked `verified: true`, contain at least two queries, and contain required evidence matching its printed page label.
...
The compiler checks the PDF SHA-256, page count, every page label, heading-tree levels, generated ID collisions, complete retrieval plans, and all strict schemas.

It also compares every Manifest heading text, page index, bounding box, numbered state, and printed parent with every source-PDF heading candidate assigned to a printed section. Missing or extra headings prevent index generation.
